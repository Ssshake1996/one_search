...
assert.deepEqual(Object.keys(managed).sort(), ['dataDir', 'installDir', 'noAutostart', 'releaseDir', 'roots', 'skipModel']);
assert.equal(managed.skipModel, true);
assert.equal(managed.noAutostart, true);
assert.ok(managed.roots.length > 0);
for (const path of [request.home, managed.installDir, managed.dataDir, ...managed.roots]) inside(path, fixtureRoot, 'Only fixture paths');
assert.equal(resolve(managed.releaseDir), resolve(request.bundleDir, '..', '..'));
for (const root of managed.roots) inside(await realpath(root), await realpath(fixtureRoot), 'Synthetic roots only');
for (const path of [managed.installDir, managed.dataDir]) {
  try { await access(path); throw new Error('Managed auto-install requires new install/data directories'); }
  catch (error) { if (error.code !== 'ENOENT') throw error; }
}
const home = resolve(request.home);
const command = resolve(managed.installDir, 'runtime', 'data-search.exe');
const commandArgs = [];
const configPath = resolve(managed.dataDir, 'config.json');
assert.match(request.profile, /^[a-zA-Z0-9_-]{1,80}$/);
try { await access(home); throw new Error('Each DSH worker requires a fresh isolated home'); }
catch (error) { if (error.code !== 'ENOENT') throw error; }
...
try {
  ...
  await registerBundle({ dshPackage: host, dshHome: home, profile: request.profile, offline: request.offline === true });
  await writeFile(join(home, 'profiles', request.profile, 'cordis.patch.yml'), JSON.stringify([
    { id: 'one-search', config: { ...managed, clientId, clientLabel: clientId } },
  ...
  assert.equal(installedConfig.scope, 'directories');
  assert.equal(installedConfig.semantic.enabled, false);
  assert.deepEqual(installedConfig.roots.map((path) => resolve(path)), managed.roots.map((path) => resolve(path)));
