...
With the 0.5.1+ DSH bundle, native upgrades automatically pause all registered MCP clients, drain runtime users, and reconnect after health checks or rollback. Older MCP clients and an open native Settings window must be closed first. Run the installer from this extracted release, keeping the same InstallDir/DataDir. After an interrupted native upgrade, retry the same installer; do not delete the maintenance marker or retained snapshots. See [upgrade instructions](docs/UPGRADE.md).
