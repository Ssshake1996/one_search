...
def _check_database(source: dict) -> dict:
    ...
    try:
        ...
        if password_env and password_env not in os.environ:
            raise DatabaseError("Configured password environment variable is missing in this process")
        with adapter._connection() as connection:
            ...
            for table in adapter.tables:
                ...
                if not set(expected).issubset(fields):
                    raise DatabaseError("An allowed column is unavailable")
                if not fields:
                    raise DatabaseError("No allowed readable columns are available")
            ...
            for entry in source.get("index", []):
                ...
                if table not in schemas:
                    raise DatabaseError("Indexed table is unavailable or not allowed")
        ...
        checks.append({"name": "validation", "ok": False, "message": str(error)[:300]})
    except Exception:
...
def check_databases(sources: list[dict], timeout_seconds: float = 30) -> dict:
    ...
    return {"preflight": True, "ok": all(item["ok"] for item in reports), "databases": reports,
            ...
            "note": "只读预检；未读取正文。环境变量仅验证本次进程，登录启动环境需单独验证。"}
