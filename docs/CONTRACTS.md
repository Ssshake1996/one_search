...
## DSH Web management and progress

The bundle declares an official `dsh.client` export and registers a `sidebar.panellist` / keyed `main` entry. `/one-search` channel, `request` endpoint accepts `{action, params}` through a `webServer.register` route guarded by `connection.requestRejection`, carrying the standard DSH RPC envelope. This avoids the installed DSH 0.1.5-rc.2 `rpc.handle` implementation's undeclared service access without patching DSH. Its application envelope is `{ok:true,result}` or `{ok:false,error:{code,message,details?}}`. The host has a fixed action allowlist, 128KiB application request limit, 2MiB backend result limit and a management queue limited to four operations. Browser requests never receive the daemon bearer token, raw configuration or stored database passwords. Headless profiles retain MCP without requiring Web services.

Status and simple indexing controls use direct authenticated loopback RPC, with an in-flight cache shared between pages. Status polling never spawns Python. Occasional settings/database/model actions invoke `data-search web-manage --config CONFIG` with bounded JSON on stdin, never a shell or password argument. The daemon intentionally continues to reject browser-origin requests.

`settings_get` returns a revision plus allowlisted editable values. Preview validates a candidate and reports bounded known-scope impact and read-only database preflight. Save rechecks the revision inside a shared settings lock, tests changed database definitions, applies configuration outside the daemon, restarts the service and attempts rollback on failure. Saved secret references are opaque; unsupported internal configuration is retained server-side. A started save is not killed when the browser disconnects. The 120-second save response deadline does not terminate the transaction; the mutation queue remains held until the child exits. An unknown-result response requires reloading configuration/status before retry.

`index_status.progress` has `schema_version:1`, `sampled_at`, `overall`, `known_unique_files`, `discovery`, `content`, `semantic`, `databases`, `resources`, `runtime_policy`, and `error_summary`. Discovery has `total:null`; root `observed_files` counts traversed records, potentially repeated after directory restart. Content counts cover files only, while semantic counts include authorized database chunks. `content.pending` includes durable retry work. `semantic.embedded/eligible` describes currently known chunks, independently from vector publication. Database progress omits cursor/key/watermark values. `overall.scope_complete` is always false: `up_to_date` means currently known work is handled, not every file on the machine is readable or indexed. Stages can overlap. Coverage counts may be cached for five seconds.

The visible panel polls around every two seconds with no overlapping requests and backs off on errors. Hidden or unmounted panels stop scheduling. Browser drafts remain separate from status snapshots. Local Web compatibility is validated against the installed DSH CLI/client versions; remote Web carriers require their own validation.
